...
class DRQN(object):
    def __init__(self, lstm_cell, scope_name, reuse_encoder_variables, input_frames=None):
        ...
        self.keep_prob = tf.placeholder(dtype=tf.float32, shape=[], name=scope_name + 'keep_prob')
    ...
    def build_graph(self):
        ...
        with tf.variable_scope(self.scope_name):
            ...
            conv_layer, conv_vars = conv_2_half_size(out, 256, scope_name=self.scope_name + 'down_sampling2')
            out = tf.nn.relu(conv_layer, name=self.scope_name + 'down_sampling_relu2')

            # out: [image_size/256, image_size/256, 256]
            ...
            # The output from the recurrent layer split into separate Value and Advantage streams
            advantage_stream, value_stream = tf.split(tf.reshape(rnn, shape=[-1, units_size]), 2, 1)
            advantage_weights = tf.get_variable(self.scope_name + 'advantage_weights', [units_size // 2, actions_num],
                                                initializer=tf.contrib.layers.xavier_initializer())
            value_weights = tf.get_variable(self.scope_name + 'value_weights', [units_size // 2, 1],
                                            initializer=tf.contrib.layers.xavier_initializer())
            advantage = tf.matmul(advantage_stream, advantage_weights)
            value = tf.matmul(value_stream, value_weights)
            ...
            Q = tf.reduce_sum(tf.multiply(self.Qout, actions_one_hot), axis=1)
    ...
    @classmethod
    def warm_lstm_state(cls, sess, net, current_lstm_state, warm_frames, keep_prob):
...
def train(studies_dir, list_of_dirs=False):
    ...
    with tf.Session() as sess:
        ...
        for i in range(epoch_num):
            ...
            lstm_state = DRQN.warm_lstm_state(sess, main_net, lstm_state, warm_frames, (1 - e) + keep_prob_factor)
            ...
            while j < max_study_len:
                ...
                if np.random.rand(1) < e or total_steps < pre_train_steps:
                    lstm_state1 = sess.run(main_net.rnn_state, feed_dict={main_net.input_frames: s,
                                                                          ...
                                                                          main_net.keep_prob: (1 - e) + keep_prob_factor,
                ...
                if total_steps > pre_train_steps:
                    ...
                    if total_steps % update_freq == 0:
                        ...
                        main_state_train = DRQN.warm_lstm_state(sess, main_net, state_train, warm_frames, (1-e) + keep_prob_factor)
                        target_state_train = DRQN.warm_lstm_state(sess, target_net, state_train, warm_frames, (1-e) + keep_prob_factor)
                        ...
                        while not done_trace:
                            ...
                            target_next_state, Q2 = sess.run([target_net.rnn_state, target_net.Qout],
                                                             feed_dict={target_net.input_frames: np.vstack(train_batch[:, 3]),
                                                                        ...
                                                                        target_net.keep_prob: (1 - e) + keep_prob_factor,
...
def test(studies_dir):
    ...
    with tf.Session() as sess:
        ...
        for i in range(num_of_studies):
            ...
            # Warm up the recurrent layer's hidden state
            lstm_state = (np.zeros([1, units_size]), np.zeros([1, units_size]))
            lstm_state = DRQN.warm_lstm_state(sess, main_net, lstm_state, warm_frames, 1)
