...
from Utilities.nn_utils import conv_relu, conv_2_half_size, fully_connected, leaky_relu, batchnorm
...
class Tracker(object):
    def __init__(self):
        ...
        self.target = tf.placeholder(tf.int32, [batch_size, 1])
    ...
    def build(self):
        with tf.variable_scope("FingersTracker"):
            with tf.variable_scope("VGG16"):
                ...
                out, _ = batchnorm(out, "batchnorm1")
                ...
                out, _ = batchnorm(out, "batchnorm2")
                ...
                out, _ = batchnorm(out, "batchnorm3")
                ...
                out, _ = batchnorm(out, "batchnorm4")
                ...
                out, _ = batchnorm(out, "batchnorm5")
            ...
            with tf.variable_scope("prediction"):
                ...
                self.predict_index = tf.argmax(self.predict)
            ...
            while not study.is_end_of_study():
                ...
                _, loss, next_state = sess.run([tracker.optimizer, tracker.loss, tracker.out_state],
                                         feed_dict={tracker.input_frames: image,
                                                    tracker.target: [direction_index],
                                                    tracker.keep_prob: keep_prob,
                                                    tracker.cell_state: current_cell_state,
                                                    tracker.hidden_state: current_hidden_state})
                current_cell_state, current_hidden_state = next_state
    ...
    with tf.Session() as sess:
        ...
        while not studies_handler.epoch_done:
            ...
            while not study.is_end_of_study():
                ...
                loss, next_state, predict_index = sess.run([tracker.loss, tracker.out_state, tracker.predict_index],
                                         feed_dict={tracker.input_frames: image,
                                                    tracker.target: [direction_index],
                                                    tracker.keep_prob: keep_prob,
                                                    tracker.cell_state: current_cell_state,
                                                    tracker.hidden_state: current_hidden_state})
                current_cell_state, current_hidden_state = next_state
                print("index: {}, predicted_index: {}, loss: {}".format(direction_index, predict_index, loss))
